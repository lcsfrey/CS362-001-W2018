...
public class TimeTableRandomTest {
    ...
    public static String RandomSelectMethod(Random random) {
        String[] methodArray = new String[] { "getApptRange", "deleteAppt" };// The list of the of
                                                                             // methods to be tested
                                                                             // in the Appt class

        int n = random.nextInt(methodArray.length);// get a random number between 0 (inclusive) and
                                                   // methodArray.length (exclusive)
    ...
    public void testRandom() throws Throwable {
        ...
        System.out.println("Start TimeTable random testing...");
            ...
            for (int iteration = 0; elapsed < TestTimeout; iteration++) {
                ...
                for (int i = 0; i < NUM_TESTS; i++) {
                    ...
                    if (methodName.equals("getApptRange")) {
                        ...
                        NumDaysInMonth = CalendarUtil.NumDaysInMonth(startYear, startMonth - 1);
                        int firstDay = ValuesGenerator.getRandomIntBetween(random, 1,
                                NumDaysInMonth);
                        GregorianCalendar firstCalDay = new GregorianCalendar(startYear, firstMonth,
                                firstDay);

                        int lastMonth = ValuesGenerator.getRandomIntBetween(random, 1, 12);
                        NumDaysInMonth = CalendarUtil.NumDaysInMonth(startYear, startMonth - 1);
                        int lastDay = ValuesGenerator.getRandomIntBetween(random, 1,
                                NumDaysInMonth);
                        GregorianCalendar lastCalDay = new GregorianCalendar(startYear, lastMonth,
                                lastDay);

                        try {
                            LinkedList<CalDay> caldays = table.getApptRange(list, firstCalDay,
                                    lastCalDay);
                            continue;
                        ...
                        if (listAppts == null) {
                            ...
                            for (int j = 0; j < listAppts.size() / 2; j++) {
                                ...
                                if (choice == 0) {
                                    ...
                                    int apptChoiceIndex = ValuesGenerator
                                            .getRandomIntBetween(random, 0, listAppts.size() - 1);
